...
This directory contains code that generates all $(\{r,m\};g)$-graphs of order $n$, given $r$, $m$, $g$ and $n$, for $n$ up to 128.
...
The generation can either be done in one or two phases.
 ...
 - One phase: one process generates all the $(\{r,m\};g)$-graphs.
...
This directory contains exhaustive lists of $(\{r,m\};g)$-cages (which are $(\{r,m\};g)$-graphs of minimum order). A file `n<n>_r<r>_m<m>_g<g>.g6` contains all the  $(\{r,m\};g)$-cages of order $n$., e.g. `n19_r3_m6_g5.g6`.
...
where
 ...
 - `<l0>`: the level $\ell_0$ in the search tree up until you want the process of the first phase to go. In case you want to run the generation in one phase, you can set this variable to a value larger then the amount of edges that the $(\{r,m\};g)$-graph could have. The number 99999999 should be fine.
 ...
 - `<res>` and `<mod>`: `<mod>` needs to be equal to the amount of processes $N$ you use for the second phase. In case `<phase>` is equal to `1`, `<res>` can be any non-negative integer. In case `<phase>` is equal to `1`, then `<res>` is equal to the process number $i \in \{0,1, \ldots, N-1\}$. Note that the values of `<res>` and `<mod>` do not matter if you only use one phase; you can use e.g. `<res>`  = `1` and `<mod>` = `1`.

Information on the program execution is sent to stderr and the generated $(\{r,m\};g)$-graphs are sent to stdout in graph6 format.

#### Example in one phase

Suppose you want to generate all the $(\{3,4\};7)$-graphs of order $n=29$ in one phase. Then you can use the following command.
```
...
Suppose you want to generate all the $(\{4,6\};5)$-graphs of order $n=25$ in two phases and want to make use of $N=5$ processes in the second phase.  Then you can use the following command for the first phase.
```
...
``` 
for each `<res>` $\in \{0,1,2,3,4\}$, which comes down to the following.
```
...
Search ended successfully
```
The stdout output of all five commands should contain all $(\{4,6\};5)$-graphs of order $25$, but not isomorphism-free. In order to only have the pairwise non-isomorphic graphs, one can use the `shortg` command from nauty (which is also available in this repository).
