...
bool bitset_empty(const bitset_t * b) {
    for(size_t k = 0; k < b->arraysize; k++) {
        if(b->array[k] != 0)
            return false;
    }
    return true;
...
void bitset_intersection(const bitset_t * CBITSET_RESTRICT b1, const bitset_t * CBITSET_RESTRICT b2,
                         bitset_t * CBITSET_RESTRICT intersecb) {
    size_t minlength = b1->arraysize < b2->arraysize ? b1->arraysize : b2->arraysize;
    size_t k = 0;
    for( ; k < minlength; ++k) {
        intersecb->array[k] = b1->array[k] & b2->array[k];
    }
    for(; k < b1->arraysize; ++k) {
        intersecb->array[k] = 0; // memset could, maybe, be a tiny bit faster
    }
}
